module ApplicationHelper
  ...
  def vote_or_unvote(article)
    ...
    if vote
      link_to('UNVOTE', article_vote_path(article_id: article.id, id: vote.id), class: 'btn btn-danger', method: :delete)
    else
      link_to('VOTE', article_votes_path(article_id: article.id), class: 'btn btn-orange', method: :post)
    end
  end  
end
